...
# Context processor to make settings and search available in all templates
@app.context_processor
def inject_settings():
    settings = Settings.query.first()
    search = request.args.get('search', None)
    return {'settings': settings, 'search': search, 'category': request.args.get('category', None)}
...
def index():
    ...
    search = request.args.get('search', '').strip()  # Strip whitespace
    ...
    if search:  # Only search if search term is not empty after stripping
    ...
    return render_template('index.html', products=products, categories=categories, search=search if search else None)
